...
NOTIFY4           = wiz.getS('notify4')
NOTEID4           = wiz.getS('noteid4')
NOTEDISMISS4      = wiz.getS('notedismiss4')
...
NOTIFICATION4    =uservar .NOTIFICATION4
...
remove_url = 'https://raw.githubusercontent.com/Kcat123/Txt/master/Remove_Add-ons.txt'

user_folder=os.path.join(xbmc.translatePath('special://masterprofile'),'addon_data')

remove_url2 = 'https://raw.githubusercontent.com/Kcat123/Txt/master/Remove_Add-ons.txt'
###########################
###### Menu Items   #######
###########################
...
def index():
	...
	addDir ('התקנות בילדים'        ,'builds',   icon=ICONBUILDS,   themeit=THEME1)
	...
	addDir ('תפריט שמירת נתונים', 'savedata', icon=ICONSAVE,     themeit=THEME1)
	if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
...
def buildMenu():
	...
	if not WORKINGURL == True:
		...
		addDir ('תפריט שמירת נתונים'       ,'savedata', icon=ICONSAVE,     themeit=THEME3)
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
...
def viewBuild(name):
	...
	for version, url, gui, kodi, themefile, icon, fanart, preview, adult, description in match:
		...
		if HIDESPACERS == 'No': addFile(wiz.sep(), '', themeit=THEME3)
		addDir ('תפריט שמירת נתונים',       'savedata', icon=ICONSAVE,     themeit=THEME3)
		addFile('מידע בילד',    'buildinfo', name, description=description, fanart=fanart, icon=icon, themeit=THEME3)
		...
		if warning == True:
			addFile('[I]זהירות! בילד זה מתוכנן לגרסת קודי %s בלבד! (במכשירך מותקנת גרסה: %s)[/I]' % (str(kodi), str(KODIV)), '', fanart=fanart, icon=icon, themeit=THEME3)
		addFile(wiz.sep(''), '', fanart=fanart, icon=icon, themeit=THEME3)
		addFile('התקנה חדשה/נקיה'   , 'install', name, 'fresh'  , description=description, fanart=fanart, icon=icon, themeit=THEME1)
		#addFile('התקנה רגילה', 'install', name, 'normal' , description=description, fanart=fanart, icon=icon, themeit=THEME1)
		if not gui == 'http://': addFile('עדכונים מהירים'    , 'install', name, 'gui'     , description=description, fanart=fanart, icon=icon, themeit=THEME1)
...
def STARTP():
	...
	if BUILDNAME == "":
	 ...
	 if not NOTIFY4 == 'true':
          url = wiz.workingURL(NOTIFICATION4)
	search_entered =input
	url = urllib2.Request(YOUTUBE)
...
def saveMenu():
	on = '[COLOR green](פעיל) [/COLOR]'; off = '[COLOR red](מנוטרל) [/COLOR]'
	...
	infowatch  = 'true' if KEEPINFOWATCH == 'true' else 'false'
	settings   = 'true' if KEEPSETTINGS  == 'true' else 'false'
	...
	addDir ('שמירת נתוני Trakt',               'trakt',                icon=ICONTRAKT, themeit=THEME1)
	addDir ('שמירת נתוני Real Debrid',              'realdebrid',           icon=ICONREAL,  themeit=THEME1)
	addFile('- Click to toggle settings -', '', themeit=THEME3)
	addFile('שמירת חשבון Trakt: %s' % trakt.replace('true',on).replace('false',off)                       ,'togglesetting', 'keeptrakt',      icon=ICONTRAKT, themeit=THEME1)
	addFile('שמירת חשבון Real Debrid: %s' % real.replace('true',on).replace('false',off)                  ,'togglesetting', 'keepdebrid',     icon=ICONREAL,  themeit=THEME1)
	addFile('שמירת מקורות: %s' % sources.replace('true',on).replace('false',off)           ,'togglesetting', 'keepsources',    icon=ICONSAVE,  themeit=THEME1)
	addFile('שמירת מידע צפיות: %s' % infowatch.replace('true',on).replace('false',off)           ,'togglesetting', 'keepinfowatch',    icon=ICONSAVE,  themeit=THEME1)
	addFile('שמירת הגדרות קודי: %s' % settings.replace('true',on).replace('false',off)               ,'togglesetting', 'keepsettings',    icon=ICONSAVE, themeit=THEME1)
	addFile('שמירת פרופילים: %s' % profiles.replace('true',on).replace('false',off)         ,'togglesetting', 'keepprofiles',   icon=ICONSAVE,  themeit=THEME1)
	addFile('שמירת קובץ Advancedsettings.xml: %s' % advanced.replace('true',on).replace('false',off) ,'togglesetting', 'keepadvanced',   icon=ICONSAVE,  themeit=THEME1)
	addFile('שמירת מועדפים: %s' % favourites.replace('true',on).replace('false',off)     ,'togglesetting', 'keepfavourites', icon=ICONSAVE,  themeit=THEME1)
	addFile('שמירת סופר מועדפים: %s' % super.replace('true',on).replace('false',off)            ,'togglesetting', 'keepsuper',      icon=ICONSAVE,  themeit=THEME1)
	addFile('שמירת מאגרי ריפו: %s' % repos.replace('true',on).replace('false',off)           ,'togglesetting', 'keeprepos',      icon=ICONSAVE,  themeit=THEME1)
	addFile('שמירת הרחבות: %s' % whitelist.replace('true',on).replace('false',off)        ,'togglesetting', 'keepwhitelist',  icon=ICONSAVE,  themeit=THEME1)
	if whitelist == 'true':
		addFile('עריכת רשימת הרחבות לשמירה',        'whitelist', 'edit',   icon=ICONSAVE,  themeit=THEME1)
		addFile('צפיה ברשימת הרחבות שמורות',        'whitelist', 'view',   icon=ICONSAVE,  themeit=THEME1)
		addFile('ניקוי רשימת הרחבות שמורות',       'whitelist', 'clear',  icon=ICONSAVE,  themeit=THEME1)
		addFile('יבוא רשימת הרחבות לשמירה',      'whitelist', 'import', icon=ICONSAVE,  themeit=THEME1)
		addFile('יצוא רשימת הרחבות שמורות',      'whitelist', 'export', icon=ICONSAVE,  themeit=THEME1)
	setView('files', 'viewType')
...
def buildWizard(name, type, theme=None, over=False):
	...
	if type == 'gui':
		if name == BUILDNAME:
			if over == True: yes = 1
			else: yes = DIALOG.yesno(ADDONTITLE, '[COLOR %s]האם להחיל את העדכונים לבילד:' % COLOR2, '[COLOR %s]%s[/COLOR]?[/COLOR]' % (COLOR1, name), nolabel='[B][COLOR red]לא, ביטול[/COLOR][/B]',yeslabel='[B][COLOR green]החל עדכונים[/COLOR][/B]')
		else: 
			yes = DIALOG.yesno("%s - [COLOR red]אזהרה!![/COLOR]" % ADDONTITLE, "[COLOR %s][COLOR %s]%s[/COLOR]לא נמצאה גרסת בילד " % (COLOR2, COLOR1, name), "האם ברצונך להחיל את העדכונים בכל מקרה? (לא מומלץ)![/COLOR]", nolabel='[B][COLOR red]לא, ביטול[/COLOR][/B]',yeslabel='[B][COLOR green]החל עדכונים[/COLOR][/B]')
		if yes:
			remove_addons()
			remove_addons2()
			...
			else: DIALOG.ok(ADDONTITLE, "[COLOR %s]העדכונים בוצעו בהצלחה! לשמירת השינויים ויציאה לחץ אישור![/COLOR]" % COLOR2); wiz.killxbmc('true')
		else:
		...
		if warning == True:
			...
			else: yes_pressed = DIALOG.yesno(ADDONTITLE, '[COLOR %s]התקנה רגילה! (נתונים והרחבות ישמרו). האם להמשיך בהתקנת' % COLOR2, '[COLOR %s]%s v%s[/COLOR]?[/COLOR]' % (COLOR1, name, wiz.checkBuild(name,'version')), nolabel='[B][COLOR red]ביטול[/COLOR][/B]',yeslabel='[B][COLOR green]התקנה[/COLOR][/B]')
		if yes_pressed:
			wiz.clearS('build')
			...
			if int(float(percent)) > 0:
				...
				else: DIALOG.ok(ADDONTITLE, "[COLOR %s]ההתקנה הסתיימה בהצלחה! לחץ אישור לסגירת תוכנית ההתקנה![/COLOR]" % COLOR2); wiz.killxbmc('true')
				if todo == 1: wiz.reloadFix()
		...
		if installtheme:
			...
			if url not in ["fresh", "normal"]:
				test = testTheme(lib) if not wiz.currSkin() in ['skin.confluence', 'skin.estuary', 'skin.aura', 'skin.eminence.2.mod', 'skin.eminence.2.mod.Krypton', 'skin.fuse.neue'] else False
				test2 = testGui(lib) if not wiz.currSkin() in ['skin.confluence', 'skin.estuary', 'skin.aura', 'skin.eminence.2.mod', 'skin.eminence.2.mod.Krypton', 'skin.fuse.neue'] else False
				if test == True:
					wiz.lookandFeelData('save')
					skin     = 'skin.confluence' if KODIV < 17 else 'skin.estuary' if KODIV < 17 else 'skin.aura' if KODIV < 17 else 'skin.eminence.2.mod' if KODIV < 17 else 'skin.eminence.2.mod.Krypton' if KODIV < 17 else 'skin.fuse.neue'
...
def toggleCache(state):
    cachelist = ['includevideo', 'includeall', 'includeexodusredux', 'includegaia', 'includethecrew', 'includeseren']
    titlelist = ['Include Video Addons', 'Include All Addons', 'Include Exodus Redux', 'Include Gaia', 'Include The Crew', 'Include Seren']
...
def restoreit(type):
    ...
    if not wiz.currSkin() in ['skin.confluence', 'skin.estuary', 'skin.aura', 'skin.eminence.2.mod', 'skin.eminence.2.mod.Krypton', 'skin.fuse.neue']:
...
def freshStart(install=None, over=False):
	...
	elif install == 'restore': yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]האם ברצונך לשחזר את" % COLOR2, "תצורת Kodi להגדרות ברירת המחדל", "לפני התקנת הגיבוי?[/COLOR]", nolabel='[B][COLOR red]ביטול[/COLOR][/B]', yeslabel='[B][COLOR green]המשך[/COLOR][/B]')
	elif install: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]התקנה חדשה/נקיה!" % COLOR2, "כל הנתונים וההגדרות ימחקו (למעט שמירות נתונים שהופעלו)!", "[COLOR %s]%s[/COLOR]האם להמשיך בהתקנת ?" % (COLOR1, install), nolabel='[B][COLOR red]ביטול[/COLOR][/B]', yeslabel='[B][COLOR green]התקנה[/COLOR][/B]')
	else: yes_pressed=DIALOG.yesno(ADDONTITLE, "[COLOR %s]האם ברצונך לשחזר את" % COLOR2, "תצורת Kodi להגדרות ברירת המחדל?[/COLOR]", nolabel='[B][COLOR red]לא, ביטול[/COLOR][/B]', yeslabel='[B][COLOR green]המשך[/COLOR][/B]')
	if yes_pressed:
		if not wiz.currSkin() in ['skin.confluence', 'skin.estuary', 'skin.aura', 'skin.eminence.2.mod', 'skin.eminence.2.mod.Krypton', 'skin.fuse.neue']:
			skin = 'skin.confluence' if KODIV < 17 else 'skin.estuary' if KODIV < 17 else 'skin.aura' if KODIV < 17 else 'skin.eminence.2.mod' if KODIV < 17 else 'skin.eminence.2.mod.Krypton' if KODIV < 17 else 'skin.fuse.neue'
		...
		if KEEPINFOWATCH == 'true':
			EXCLUDES.append('script.module.metahandler')
		...
		for root, dirs, files in os.walk(xbmcPath,topdown=True):
			...
			for name in files:
				...
				elif name == 'MyVideos107.db' and fold[-1] == 'Database' and KEEPINFOWATCH == 'true': wiz.log("Keep Info Watch: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
				elif name == 'MyVideos116.db' and fold[-1] == 'Database' and KEEPINFOWATCH == 'true': wiz.log("Keep Info Watch: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
				elif fold[x-2] == 'userdata' and fold[x-1] == 'addon_data' and 'script.module.metahandler' in fold[x]  and KEEPINFOWATCH =='true' : wiz.log("Keep Info Watch: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
				elif name == 'guisettings.xml' and fold[-1] == 'userdata' and KEEPSETTINGS == 'true': wiz.log("Keep Settings: %s" % os.path.join(root, name), xbmc.LOGNOTICE)
...
def testnotify4():
	...
	if url == True:
		try:
			id, msg = wiz.splitNotify(NOTIFICATION4)
			if id == False: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]אין עדכון מהיר[/COLOR]" % COLOR2); return
			notify.notification4(msg, True)
		except Exception, e:
			wiz.log("Error on Notifications Window: %s" % str(e), xbmc.LOGERROR)
	else: wiz.LogNotify("[COLOR %s]%s[/COLOR]" % (COLOR1, ADDONTITLE), "[COLOR %s]אין עדכון מהיר[/COLOR]" % COLOR2)
def testupdate():
