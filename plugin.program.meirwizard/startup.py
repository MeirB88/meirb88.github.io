...
NOTEID4        = 0 if NOTEID4 == "" else int(NOTEID4)
...
if ENABLE == 'Yes' and BUILDNAME == "KodiCat - Eminence Mod 17":
	if not NOTIFY == 'true':
...
if ENABLE == 'Yes' and BUILDNAME == "KodiCat - Eminence Mod 18":
	if not NOTIFY2 == 'true':
...
if ENABLE == 'Yes' and BUILDNAME == "KodiCat Aura 18":
	if not NOTIFY3 == 'true':
...
wiz.log("[Notifications4] Started", xbmc.LOGNOTICE)
if ENABLE == 'Yes' and BUILDNAME == "KodiCat - Fuse 18":
	if not NOTIFY4 == 'true':
		...
		if url == True:
			...
			if not id == False:
				try:
					id = int(id); NOTEID4 = int(NOTEID4)
					if id == NOTEID4:
						if NOTEDISMISS4 == 'false':
							notify.notification4(msg)
						else: wiz.log("[Notifications4] id[%s] Dismissed" % int(id), xbmc.LOGNOTICE)
					elif id > NOTEID4:
						wiz.log("[Notifications4] id: %s" % str(id), xbmc.LOGNOTICE)
						wiz.setS('noteid4', str(id))
						wiz.setS('notedismiss4', 'false')
						notify.notification4(msg=msg)
						wiz.log("[Notifications4] Complete", xbmc.LOGNOTICE)
				except Exception, e:
					wiz.log("Error on Notifications Window: %s" % str(e), xbmc.LOGERROR)
			else: wiz.log("[Notifications4] Text File not formated Correctly")
		else: wiz.log("[Notifications4] URL(%s): %s" % (NOTIFICATION4, url), xbmc.LOGNOTICE)
	else: wiz.log("[Notifications4] Turned Off", xbmc.LOGNOTICE)
else: wiz.log("[Notifications4] Not Enabled", xbmc.LOGNOTICE)
