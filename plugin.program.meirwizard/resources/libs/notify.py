		...
		def doWrite(self):
			...
			if KODIV < 17:
				...
				with open(ADVANCED, 'w+') as f:
					...
					f.write('		<curllowspeedtime>%s</curllowspeedtime>\n' % self.currentCurl)
					f.write('	</network>\n')
					f.write('	<imageres>720</imageres>\n')
					f.write('	<fanartres>1080</fanartres>\n')
					f.write('	<packagefoldersize>200</packagefoldersize>\n')
					f.write('</advancedsettings>\n')
				f.close()
			self.CloseWindow()
			wiz .LogNotify ("[COLOR %s]%s[/COLOR]"%(COLOR1 ,ADDONTITLE ),'[COLOR %s]לשמירה וטעינת ההגדרות יש לאתחל את Kodi![/COLOR]'%COLOR2 )
...
def firstRunSettings():
	class firstRun(xbmcgui.WindowXMLDialog):
		...
		def onInit(self):
			...
			self.thumbs     = 313
			self.showdialog()
			self.controllist     = [self.trakt, self.debrid, self.infowatch, 
			...
			self.controlsettings = ['keeptrakt', 'keepdebrid', 'keepinfowatch',
...
def firstRun():
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			self.title = "[COLOR gold]ברוכים הבאים ל-MEIR Wizard[/COLOR]"
			self.msg   ='''\
             מגוון בילדים בעברית לקודי
                '''
...
def notification(msg='', test=False):
	class MyWindow(xbmcgui.WindowXMLDialog):
		...
		def doFastupdate(self):
			...
			wiz.ebi("PlayMedia(plugin://%s/?mode=install&name=%s&url=gui)" % (ADDON_ID, urllib.quote_plus(BUILDNAME))); wiz.log("[Installed Check] Guifix attempting to install")
			self.close()
...
def notification2(msg='', test=False):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			self.test = kwargs['test']
			self.message =  THEME2 % kwargs['msg']
		...
		def doFastupdate(self):
			...
			wiz.log("[Notification2] NotifyID2 %s Dismissed" % wiz.getS('noteid2'), xbmc.LOGNOTICE)
			wiz.ebi("PlayMedia(plugin://%s/?mode=install&name=%s&url=gui)" % (ADDON_ID, urllib.quote_plus(BUILDNAME))); wiz.log("[Installed Check] Guifix attempting to install")
			self.close()
		def doRemindMeLater(self):
			if not test == True:
				wiz.setS("notedismiss2","false")
			wiz.log("[Notification2] NotifyID2 %s Remind Me Later2" % wiz.getS('noteid2'), xbmc.LOGNOTICE)
			self.close()
	...
	xbmc.executebuiltin('Skin.SetString(headertexttype, %s)' % 'true' if HEADERTYPE == 'Text' else 'false')
	xbmc.executebuiltin('Skin.SetString(headerimagetype, %s)' % 'true' if HEADERTYPE == 'Image' else 'false')
	notify2 = MyWindow( "Notifications.xml" , ADDON.getAddonInfo('path'), 'DefaultSkin', msg=msg, test=test)
	notify2.doModal()
	del notify2
	
def notification3(msg='', test=False):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			self.test = kwargs['test']
			self.message =  THEME2 % kwargs['msg']
		...
		def doFastupdate(self):
			if not test == True:
				wiz.setS("notedismiss3","true")
			wiz.log("[Notification3] NotifyID3 %s Dismissed" % wiz.getS('noteid3'), xbmc.LOGNOTICE)
			wiz.ebi("PlayMedia(plugin://%s/?mode=install&name=%s&url=gui)" % (ADDON_ID, urllib.quote_plus(BUILDNAME))); wiz.log("[Installed Check] Guifix attempting to install")
			self.close()
		def doRemindMeLater(self):
			if not test == True:
				wiz.setS("notedismiss3","false")
			wiz.log("[Notification3] NotifyID3 %s Remind Me Later2" % wiz.getS('noteid3'), xbmc.LOGNOTICE)
			self.close()
	...
	xbmc.executebuiltin('Skin.SetString(headertexttype, %s)' % 'true' if HEADERTYPE == 'Text' else 'false')
	xbmc.executebuiltin('Skin.SetString(headerimagetype, %s)' % 'true' if HEADERTYPE == 'Image' else 'false')
	notify3 = MyWindow( "Notifications.xml" , ADDON.getAddonInfo('path'), 'DefaultSkin', msg=msg, test=test)
	notify3.doModal()
	del notify3

def notification4(msg='', test=False):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			self.test = kwargs['test']
			self.message =  THEME2 % kwargs['msg']
		...
		def doFastupdate(self):
			if not test == True:
				wiz.setS("notedismiss4","true")
			wiz.log("[Notification4] NotifyID4 %s Dismissed" % wiz.getS('noteid4'), xbmc.LOGNOTICE)
			wiz.ebi("PlayMedia(plugin://%s/?mode=install&name=%s&url=gui)" % (ADDON_ID, urllib.quote_plus(BUILDNAME))); wiz.log("[Installed Check] Guifix attempting to install")
			self.close()
		def doRemindMeLater(self):
			if not test == True:
				wiz.setS("notedismiss4","false")
			wiz.log("[Notification4] NotifyID4 %s Remind Me Later2" % wiz.getS('noteid4'), xbmc.LOGNOTICE)
			self.close()
	...
	xbmc.executebuiltin('Skin.SetString(headertexttype, %s)' % 'true' if HEADERTYPE == 'Text' else 'false')
	xbmc.executebuiltin('Skin.SetString(headerimagetype, %s)' % 'true' if HEADERTYPE == 'Image' else 'false')
	notify4 = MyWindow( "Notifications.xml" , ADDON.getAddonInfo('path'), 'DefaultSkin', msg=msg, test=test)
	notify4.doModal()
	del notify4
...
def updateWindow(name='Testing Window', current='1.0', new='1.1', icon=ICON, fanart=FANART):
	class MyWindow(xbmcgui.WindowXMLDialog):
		def __init__(self, *args, **kwargs):
			...
			self.fanart = kwargs['fanart']
			self.msgupdate  = "עדכון גרסה חדשה זמינה לבילד:\n[COLOR %s]%s[/COLOR]\n\nגרסת בילד נוכחית:\nv[COLOR %s]%s[/COLOR]\nגרסת בילד חדשה:\nv[COLOR %s]%s[/COLOR]\n\n[COLOR %s]*המלצה - הפעלת שמירת נתונים (במידת הצורך) לפני עדכון הבילד.\nלאחר מכן ניתן להתקין את הבילד דרך אריח שירות/תחזוקה.[/COLOR]" % (COLOR1, self.name, COLOR1, self.current, COLOR1, self.new, COLOR1)
		...
		def doSavedata(self):
			xbmc.executebuiltin( "XBMC.ActivateWindow(10001,plugin://plugin.program.KodiCatWizard/?mode=savedata,return)" )
			self.close()
